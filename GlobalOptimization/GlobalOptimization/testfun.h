...
namespace testfun
{
	...
	const vector<double> lb = { -5.12, -2.048, -1.28, -5.12, -500, -600, -30, -2, -5, -100, -10, -1, -1, -512, -10, -10, -10, -500, -pi, -10 }; /*левые границы области определени¤*/
	const vector<double> rb = { 5.12, 2.048, 1.28, 5.12, 500, 600, 30, 2, 5, 100, 10, 4, 1, 512, 10, 10, 10, 500, pi, 10 }; /*правые границы бласти определени¤*/ /*=> область определени¤*/
	...
	/* нормальное распределение - распределение √аусса */
	double gauss()
	{
	...
	double f_schwefel(const int &n, const point &x)
	{
		...
		for (int i = 0; i < n; i++)
		{
			if ((x[i]>testfun::rb[4]) || (x[i]<testfun::lb[4]))
				flag = false;                 /*flag=false, если x[i] вышла за область определени¤*/
		}
		if (flag)
		{
			for (int i = 0; i < n; i++)
	...
	{  /*вычисл¤ет степени дл¤ функции Ёкли*/
		double s1 = 0;
		double s2 = 0;
		for (int i = 0; i < n; i++)
		{
